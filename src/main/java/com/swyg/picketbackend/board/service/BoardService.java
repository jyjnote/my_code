...
//        // 추천 게시물 목록 가져오기
//        // 이 부분에서는 target의 벡터 데이터를 사용하여 추천 게시물을 조회합니다.
//        List<Board> recommendedPosts = new ArrayList<>();
//        try {
//            List<Long> recommendedPostIds = getRecommendedPosts(boardId);
//
//            recommendedPosts = boardRepository.findAllById(recommendedPostIds);
//        }catch (IOException e) {
//            // 로그 남기기
//            log.error("Error while fetching recommended posts", e);
//        }
//
//        return GetBoardDetailsResponseDTO.toDTOList(target,recommendedPosts);
//
//    }
    ...
    public GetBoardDetailsResponseDTO detailBoard(Long boardId) {
        ...
        return GetBoardDetailsResponseDTO.toDTOList(target, simpleRecommendedPosts); // 변환된 리스트를 전달
    }
    ...
    private String runPythonScript(String text) {
        try {
            String pythonScriptPath = "C:\\Users\\jaeyeon\\Downloads\\PicketBackend\\PicketBackend-jiwoong\\src\\script\\vectorize.py"; // 파이썬 스크립트 파일 경로
    ...
    private List<Long> runPythonScriptForRecommendation(Long basePostId) throws IOException {
        String pythonScriptPath = "C:\\Users\\jaeyeon\\Downloads\\PicketBackend\\PicketBackend-jiwoong\\src\\script\\recommend.py";
        String pythonExecutablePath = "C:\\Users\\jaeyeon\\anaconda3\\envs\\transformers_env\\python.exe";
